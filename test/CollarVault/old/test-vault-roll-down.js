...
describe("CollarVault and Roll", function () {
    ...
    this.beforeEach(async function (){ //gets you a brand new contract for each test 
        ...
        await usdc.connect(whaleusdc).transfer(collarVault.address,13991951*2); //vaults proceeds
        await usdc.connect(whaleusdc).transfer(joe.address,99109651*2); //joes loan
    ...
    it("executeRoll - lower", async function () {
        //const vaultdeets1 = (await collarVault.connect(joe).getVaultDetails());
        ...
        assert.equal(vaultdeets[1].toString(),String(103320050)); // lent (unch)
    ...
    it("executeRoll - lower", async function () {
        ...
        await collarVault.connect(joe).postCash(100*1e6); // post cash
        await collarVault.connect(mike).showRollPrice(110);
        await collarVault.connect(joe).giveRollOrder();
        await usdc.connect(mike).approve(collarVault.address,10000*1e6); //10k approved, 6dp
        await collarVault.connect(mike).executeRoll();
