...
describe("CollarVault Mature Up", function () {
    ...
    this.beforeEach(async function (){ //gets you a brand new contract for each test 
        ...
        await usdc.connect(whaleusdc).transfer(collarVault.address,(139919507+1)/2); //vaults proceeds
        await usdc.connect(whaleusdc).transfer(joe.address,(991096509+1)/2); //joes loan
    ...
    it("matureVault - netsh|phys, no repay, btwn - should pay some eth", async function () {
        ...
        assert.equal((await usdc.connect(mike).balanceOf(mike.address)).toString(),"1000000000000"); //no collat for mm
        assert.equal((await ethers.provider.getBalance(joe.address)).toString(),"10000010178204823562330");//joe gets back entire vault balance as eth, so his orig eth (~.5 + ~.5 he made)
