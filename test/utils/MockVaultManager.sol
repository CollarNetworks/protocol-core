...
contract MockVaultManager is ICollarVaultManager {
    ...
    function previewRedeem(bytes32, /* uuid */ uint256 /* amount */ ) external override returns (uint256) {
        return 0;
    }

    function withdraw(bytes32, /* uuid */ uint256 /* amount */ ) external override { }

    function vaultInfo(bytes32 /* uuid */ ) external view override returns (bytes memory data) {
        data = bytes("a;lsdkfj");
    }
}
