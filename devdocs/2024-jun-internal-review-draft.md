...
- #med `createVaultManager` allowing one manager per sender:
  ...
  - #med `createVaultManager` allowing only one manager per sender limits composability since cannot be used by other protocols in a flexible way
...
- #naming: `dexRouter` should have correct name (`unitV3router` or smth) because is later used with specific UniV3 interface (in vault manager)
...
- #naming "engine" is a misleading name - there's no logic really, only vault factory and config.

### CollarVaultManager

- #critical `openVault` swap price (and so the price used for accounting) is at full user control (can sandwich themselves) so user can choose any price they like:
  - can be exploited by using low price - put the provider at a loss from the start
  - should use TWAP price when opening, but with slippage protection
- #high `user` and `owner` being initially the same, but `user` being immutable and used for auth, while `owner` is transferrable is problematic and confusing.
  - #high if ownership is transferred, user is still the expected sender for sensitive functions
  - Also, engine should not store and map senders to vault-managers, since its mapping will be out of date.
  - Either Ownable should be removed, or  `onlyOwner` should be used for auth.
...
- #naming "vault" for internal struct is bad name since typically refers to a specific type of separate contract, but here it isn't a contract, it's a "position" type thing. Can be named BorrowPositions or smth similar.
- #note magic numbers: `15 minutes`, 10_000, 3000
- #note what is `VaultForABI` event??
- #naming `vaultNonce` - "nonce" is not typically used this way, can be vaultIndex
...
- #low position doesn't store the opening vault-manager, and `finalizePosition` doesn't check the closing and opening manager is the same. position should store and check the "owner" vault?
- `_getSmallestProvider`
  ...
  - #low magic number 5 is very error prone since makes sense to be easily changed
...
- #naming "slot" is an overloaded name (because of storage slots, `.slot`, and other various slots). Tick also already refers to a specific UniV3 things. So maybe "Range" / "offerRange" ?
...
- #naming `providers` should be `providersLiquidity` because it's a map
...
- #high `twapLength` is added twice (and `twapStartTimestamp` is outside of the twap window). Both incorrect price (15 minutes in past), and incorrect window is used.
- #med `twapLength` 0 should not be allowed and slot0 (instaneous price) should not be used
...
- #med `getQuoteAtTick` assumes `baseToken` is 1e18 decimals
...
- #note fee tier magic number (3000) should be named constant
...
- #issue is the library audited / secure?
...
# Already accepted (part of "review" branch)

- Refactor of Interfaces vs. Abstract confusion
- Removal of event-only interfaces
- `uint` style
- Library function should be internal
