...
contract FunctionsDONSetup is FunctionsRouterSetup {
    ...
    function setUp() public virtual override {
        ...
        // set OCR config
        s_functionsCoordinator.setConfig(s_signers, s_transmitters, s_f, s_onchainConfig, s_offchainConfigVersion, s_offchainConfig);
    }
}
...
contract FunctionsClientRequestSetup is FunctionsSubscriptionSetup {
    ...
    function _sendAndStoreRequest(
        ...
        bytes32 requestId = s_functionsClient.sendRequest(s_donId, sourceCode, secrets, args, bytesArgs, s_subscriptionId, callbackGasLimit);
    ...
    function _buildReport(uint256[] memory requestNumberKeys, string[] memory results, bytes[] memory errors)
        internal
        view
        returns (bytes memory report, bytes32[3] memory reportContext)
    {
        // Build report
    ...
    function _signReport(bytes memory report, bytes32[3] memory reportContext, uint256[] memory signerPrivateKeys)
        internal
        pure
    ...
    function _reportAndStore(
        ...
        (bytes32[] memory rawRs, bytes32[] memory rawSs, bytes32 rawVs) = _signReport(report, reportContext, signerPrivateKeys);
        ...
        if (expectedToSucceed) {
            ...
            (uint96 totalCostJuels,,,,,) = abi.decode(
                vm.getRecordedLogs()[requestProcessedIndex].data, (uint96, address, FunctionsResponse.FulfillResult, bytes, bytes, bytes)
            );
    ...
    /// @dev @param requestProcessedIndex is overloaded to give requestProcessedIndex as 3 (happy path value)] - On a successful fulfillment the Router will emit a RequestProcessed event. To grab that event we must know the order at which this event was thrown in the report transmission lifecycle. This can change depending on the test setup (e.g. the Client contract gives an extra event during its callback)
    /// @dev @param transmitterGasToUse is overloaded to give transmitterGasToUse as 0] - Sends the `.report` transaction with the default amount of gas
    function _reportAndStore(uint256[] memory requestNumberKeys, string[] memory results, bytes[] memory errors, address transmitter)
        internal
    {
