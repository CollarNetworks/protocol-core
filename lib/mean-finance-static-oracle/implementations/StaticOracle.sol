...
contract StaticOracle is IStaticOracle {
    ...
    function isPairSupported(address _tokenA, address _tokenB) external view override returns (bool) {
        ...
        for (uint256 i; i < _length; ++i) {
            address _pool =
                PoolAddress.computeAddress(address(UNISWAP_V3_FACTORY), PoolAddress.getPoolKey(_tokenA, _tokenB, _knownFeeTiers[i]));
    ...
    /// @inheritdoc IStaticOracle
    function quoteAllAvailablePoolsWithTimePeriod(uint128 _baseAmount, address _baseToken, address _quoteToken, uint32 _period)
        external
        view
        override
    ...
    /// @inheritdoc IStaticOracle
    function prepareSpecificFeeTiersWithTimePeriod(address _tokenA, address _tokenB, uint24[] calldata _feeTiers, uint32 _period)
        external
        override
        returns (address[] memory _preparedPools)
    {
    ...
    /// @inheritdoc IStaticOracle
    function prepareSpecificFeeTiersWithCardinality(address _tokenA, address _tokenB, uint24[] calldata _feeTiers, uint16 _cardinality)
        public
        override
        returns (address[] memory _preparedPools)
    {
    ...
    function _quote(uint128 _baseAmount, address _baseToken, address _quoteToken, address[] memory _pools, uint32 _period, uint32 _offset)
        internal
        view
        returns (uint256 _quoteAmount)
    {
        ...
        console.log("Pools length: ", _pools.length);
        // console.log("tickData", _tickData[0].tick, _tickData[0].weight);
        ...
        console.log("tickData.length", _tickData.length);
        console.log("tickData[0].tick:");
        console.logInt(_tickData[0].tick);
        int24 _weightedTick = _tickData.length == 1 ? _tickData[0].tick : OracleLibrary.getWeightedArithmeticMeanTick(_tickData);
        console.log("weightedTick: ");
        console.logInt(_weightedTick);
        console.log("baseAmount: ", _baseAmount);

        uint256 quote = OracleLibrary.getQuoteAtTick(_weightedTick, _baseAmount, _baseToken, _quoteToken);
        console.log("quoteAmount: ", quote);
        return quote;
    }
