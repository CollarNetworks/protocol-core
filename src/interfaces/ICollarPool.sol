...
interface ICollarPool {
    ...
    function getInitializedSlotIndices() external view virtual returns (uint[] calldata);
    ...
    function getLiquidityForSlot(uint slotIndex) external virtual returns (uint amount);

    /// @notice Gets the number of providers in a particular slot
    /// @param slotIndex The index of the slot to get the state of
    function getNumProvidersInSlot(uint slotIndex) external virtual returns (uint amount);
    ...
    function addLiquidityToSlot(uint slot, uint amount) external virtual;

    /// @notice Removes liquidity from a given slot
    /// @param slot The index of the slot to remove liquidity from
    /// @param amount The amount of liquidity to remove
    function withdrawLiquidityFromSlot(uint slot, uint amount) external virtual;
    ...
    function moveLiquidityFromSlot(uint source, uint destination, uint amount) external virtual;
    ...
    function openPosition(bytes32 uuid, uint slotIndex, uint amount, uint expiration) external virtual;
    ...
    function finalizePosition(bytes32 uuid, address vaultManager, int positionNet) external virtual;
