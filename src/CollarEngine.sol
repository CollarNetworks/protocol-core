...
import "interfaces/ICollarEngineEvents.sol";
    ...
    ICollarEngineEvents
{
    ...
    constructor(
        ...
        require(_weth != address(0) && _oracle != address(0) && _dexRouter != address(0) && _lendAsset != address(0) && _marketmaker != address(0) && _feeWallet != address(0) && _feeRatePct != 0 , "error - no zero addresses");
        admin = msg.sender;
    ...
    modifier onlyAdmin() {require(msg.sender == admin,"error - only callable by admin");_;}
    /// @notice this modifier limits calls to the specified marketmaker i.e. the deployer of this contract
    modifier onlyMarketMaker() {require(msg.sender == marketmaker,"error - only callable by whitelisted marketmaker");_;}
    ...
    function getDexRouter() external view returns (address) {return address(dexRouter);}
    function getMarketmaker() external view returns (address) {return marketmaker;}
    function getFeerate() external view returns (uint256) {return feeRatePct;}
    function getFeewallet() external view returns (address) {return feeWallet;}
    function getPricingByClient(address client) external view returns (Pricing memory) {return pricings[client];}
    function getStateByClient(address client) external view returns (PxState) {return pricings[client].state;}
    function getLendAsset() external view returns (address) {return lendAsset;}
    function getLastTradeVault(address client) external view returns (address) {return userVaults[client][nextUserVaultId[client]-1];}
    function getLastTradeVaultMarketmaker(address _marketmaker) external view returns (address) {return marketmakerVaults[_marketmaker][nextMarketmakerVaultId[marketmaker]-1];}
    function getClientEscrow(address client) external view returns (uint256) {return clientEscrow[client];}
    function getNextUserVaultId(address _client) external view returns (uint256) {return nextUserVaultId[_client];}
    function getNextMarketmakerVaultId(address _marketmaker) external view returns (uint256) {return nextMarketmakerVaultId[_marketmaker];}
    function getClientVaultById(address _client, uint256 _id) external view returns (address) {return userVaults[_client][_id];}
    function getMarketmakerVaultById(address _marketmaker, uint256 _id) external view returns (address) {return marketmakerVaults[_marketmaker][_id];}
    ...
    function getCurrRfqid() external view returns (uint256) {return rfqid;}
    
    /// @notice Retrieves the client's pricing details
    /// @return a tuple of length 12
    function getMyPrice() external view returns (uint256, address,address,PxState,string memory,string memory,uint256,uint256,uint256,uint256,uint256 ,string memory) {
        require(isCustomer[msg.sender], "error - no pricings req'd for customer");
        Pricing memory p = pricings[msg.sender];
        return (
            p.rfqid, p.lendAsset, p.marketmaker, p.state, p.structure, 
            p.underlier, p.maturityTimestamp, p.qty, p.ltv, 
            p.putstrikePct, p.callstrikePct, p.notes);
    }
    ...
    function getOraclePrice() external view returns (uint256) {return fetchOraclePrice();}
    ...
    */
    ...
    function showPrice(address _client, uint256 _callStrikePct) external onlyMarketMaker { // to add delta pct potentially
    ...
    function clientGiveOrder() external payable { // assumes mkt swaps for now
    ...
    function updateFeeRatePct(uint256 _newFeeRatePct) external onlyAdmin { // i.e. "3"
